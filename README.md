...
| Min 6 | Change 2 (Replace photos, add music): Navigate to album1, replace existing images with new ones, refresh website. | 1. `sudo git clone https://github.com/PercyChengS/Open-Day-Demo.git /tmp/demo-assets && sudo cp -r /tmp/demo-assets/SupportDoc/* /var/www/album1/images/ && sudo rm -rf /tmp/demo-assets`<br/>2. `cd /var/www/album1 && rm -rf index.html && nano index.html`<br/>3. [Replace the whole code from GitHub ↓](#min-6-replace-photos--add-music) (Can use ctrl+w to search)<br/>4. `Press Control+X, Y, Enter` |
